  ...
  return (
    <div className="min-h-screen bg-grid px-4 py-8">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        {/* Game Card */}
        <div
          className="glass-card rounded-2xl p-8 space-y-6 fade-in"
        ...
        {records.length > 0 && (
          <div
            className="glass-card rounded-2xl p-6 space-y-4 fade-in"
          ...
          >
            ...
            <div className="space-y-2">
              {records.slice(0, 5).map((record, idx) => (
                <div
                  key={record.id}
                  className="flex items-center justify-between bg-[#0a0a0f] rounded-xl px-4 py-3"
                >
                  <div className="flex items-center gap-3">
                    <span className="text-gray-500 text-sm w-6">
                      #{idx + 1}
                    </span>
                    <span className="font-mono font-bold text-[#ff9900]">
                      {formatTime(record.final_time_ms)}
                    </span>
                  </div>
                  <div className="flex items-center gap-4 text-sm text-gray-400">
                    <span>
                      {record.penalty_count > 0
                        ? `${record.penalty_count} penalties`
                        : "Perfect!"}
                    </span>
        ...
        {/* Navigation */}
        <div
          className="flex justify-center gap-4 fade-in"
